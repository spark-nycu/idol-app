...
app.post('/idol-reply', async (req, res) => {
  try {
    ...
    try {
      ...
      // Titan v1 responses look like { outputs: [ { data: "<base64>", mimeType:"image/png" } ] }
      const arr = imgJson.outputs || imgJson.results || [];
      if (arr.length && arr[0].data) {
        imageOut = arr[0].data;
      } else {
        console.error('▶️ Unexpected Titan response:', imgJson);
      }
    } catch (e) {
